...
1. **No scarcity numbers** — never show `available_seats`, "X מקומות נותרו", or fillness bars. Goal is **max registrations for data collection**. *Amended 2026-09-01:* the card shows **registered / capacity** ("12/16 זוגות רשומים") always, including on finished tournaments (muted), so a player can see the size of the draw; the "מקומות אחרונים!" badge now requires < 3 free seats instead of firing on every open tournament. The number of seats *remaining* is still never shown. See the same rule in `CLAUDE.md` for the full wording.
