...
1. **No scarcity numbers** — never render `available_seats`, "X spots left", or a fillness bar. Goal is max registrations for data collection. **Amended 2026-09-01** (product call, this rule was previously absolute): a tournament card may show *social proof* — the confirmed count — but only once the draw is at least half full (`socialProofCount` in `src/lib/tournamentHelpers.ts`), because "0 pairs are in" repels. And the "last spots" badge, which used to fire on every tournament with open registration, now fires only under 3 free seats (`isLastSpots`). Neither ever prints the seat number. Counts are **pairs**, not players, in a doubles draw — the API caps registrations, not people.
