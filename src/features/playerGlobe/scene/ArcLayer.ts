...
import {
  AMBIENT_PULSES, AMBIENT_RIVAL_PULSES, ARC_LIFT_K, ARC_LIFT_MAX, ARC_LINE_WIDTH_PX, ARC_SEG, BALL_RADIUS,
  FOCUS_PULSES,
...
export interface ArcColorPass {
  ...
  ambientAmp: number
  /** the sparse opponent pool, and its own amplitude */
  rivalPick: Picker | null
  rivalAmp: number
...
export class ArcLayer {
  ...
  /** resting traffic on the opponent links — see AMBIENT_RIVAL_PULSES */
  private readonly rivals: PulseSlot[] = makeSlots(AMBIENT_RIVAL_PULSES)
  ...
  writeColors(time: number, pass: ArcColorPass): void {
    ...
    this.stepPool(this.rivals, pass.rivalPick, time, pass.rivalAmp, null)
