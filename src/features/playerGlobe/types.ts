...
export interface GlobeNode {
  ...
  /** Verified level pair (see components/players/level's describeLevel for the shared
      3-state model this platform uses elsewhere): true = verified, false = the server
      checked and said no, null = the server did not say. An older API that sends
      neither field, or a malformed value, decodes to null via `toGlobeGraph` — never
      to a false "unverified" claim (spec: the seal is additive only). */
