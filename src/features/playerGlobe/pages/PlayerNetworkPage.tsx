...
import { GLOBE_OPEN_SOURCES, trackGlobeOpen, type GlobeOpenSource } from '@/lib/analytics'
...
export default function PlayerNetworkPage() {
  ...
  /* The card itself opens for any signed-in viewer (see `myId` above); the two
     signed-in-only REQUESTS behind it wait for a viewer whose own player profile
     exists — `useReadyViewerId` carries the reason. */
  const readyViewerId = useReadyViewerId()
  ...
  /* The door this open came from: navigation state from an in-site link first (never a query
     param, which a visitor could copy and re-share), else a link that names a player is a
     share link, else direct. Once per mount, before the graph loads — a failed load is its
     own signal, not a reason to lose the open. */
  useEffect(() => {
    const fromState = (location.state as { source?: unknown } | null)?.source
    const source: GlobeOpenSource =
      typeof fromState === 'string' && (GLOBE_OPEN_SOURCES as readonly string[]).includes(fromState)
        ? (fromState as GlobeOpenSource)
        : searchParams.get('player')
          ? 'share_link'
          : 'direct'
    trackGlobeOpen(source)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  /* `?player=<id>`: a shared card, a ranking row, a player page. Applied ONCE, when the index
     knows the id AND the session has settled — the session resolves asynchronously and often
     lands after the graph, and deciding before it lands would turn every signed-in share-link
     arrival into the signed-out one. An unknown id is ignored. Turns the ball for everyone;
     opens the card only for a signed-in viewer, because opening a player is this page's
     sign-in moment and a prompt on landing is not a welcome. */
  const pinnedRef = useRef(false)
  /* The pin's own echoes. `focusPlayer` selects the node, and the scene reports that selection
     through `onSelect` exactly as it reports a click — so signed out, the echo would open the
     sign-in gate on landing. It can echo MORE than once: a focus asked for while the layout is
     still running is parked and replayed on settle, which selects again. So the parked id is
     not consumed by echoes at all; it is cleared by the visitor's first pointer-down on the
     stage, which every real click starts with and no echo ever does. The visitor's own click
     on that same node is then the sign-in moment it should be. */
  const silentSelectRef = useRef<string | null>(null)
  useEffect(() => {
    if (pinnedRef.current || !index || sessionStatus === 'loading') return
    const id = searchParams.get('player')
    pinnedRef.current = true
    if (!id || !index.nodeById.has(id)) return
    if (myId) {
      focus(id)
    } else {
      silentSelectRef.current = id
      globeRef.current?.focusPlayer(id)
    }
  }, [index, myId, sessionStatus, searchParams, focus])
      ...
      >
        ...
        {graph && index && !empty && (
          <PlayerGlobe
            ...
            index={index}
            viewerId={myId}
            viewerLabel={t('network.youBadge')}
            ...
            onSelect={(id) => {
              ...
              if (silentSelectRef.current === id) return
              setQuery('')
              setStatus(null)
