...
export interface UsePlayerGlobeOptions {
  graph: GlobeGraph
  /** the signed-in player — their node gets the "you" marker; null when signed out */
  viewerId?: string | null
  /** the marker's label, already translated */
...
export function usePlayerGlobe(
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      scene = new GlobeScene(el, {
        ...
        images,
        background,
        viewerId: optionsRef.current.viewerId ?? null,
        viewerLabel: optionsRef.current.viewerLabel ?? null,
  ...
  /* Not in the build effect's deps on purpose: the signed-in user resolves asynchronously and
     often lands after the graph, and rebuilding the scene for it would replay the layout. */
  useEffect(() => {
    sceneRef.current?.setViewer(options.viewerId ?? null, options.viewerLabel ?? null)
  }, [options.viewerId, options.viewerLabel, ready])
