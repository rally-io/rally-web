    ...
    >
      ...
      <header className="flex items-start gap-3">
        {/* The tier ring is the ball's own TIER_COLOR, so the card and the coin the user
            ...
            DELIBERATELY CARRIES NO VERIFICATION SEAL. A `border-color` is not exposed to
            the accessibility tree at all — unlike PlayerShield's `aria-hidden` SVG, hiding
            this ring was never on the table, but a plain CSS border conveys the tier to
            sighted users only either way, exactly like PlayerShield's fill colour and
            PlayerIdentity's `MONOGRAM_TONE` (leagueRanking), both also unsealed. Platform
            convention is consistent here: tier-as-colour never carries the mark, only
            tier/level-as-words does. And the actual words claim — the level number — is
            sealed one prop below: `view` defaults to `'stats'`, so PlayerStatsTab's sealed
            level chip (see there) is the first thing visible under this very header, in
            the same card. A second seal on the ring would double-mark one player in one
            view, which is exactly the confusion PlayerShield's own comment warns about. */}
        <span
          data-testid="tier-ring"
