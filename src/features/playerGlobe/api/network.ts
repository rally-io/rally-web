...
const nodeSchema = z.object({
  ...
  // (leagueRanking/types.ts): true = verified, false = the server checked and
  // said no, null = the server did not say — an absent pair (an older backend)
  // or a malformed value both degrade to null via `.catch()`, never to a false
  // "not verified", and never throw and blank the whole globe.
...
export function toGlobeGraph(payload: NetworkPayload): GlobeGraph {
  return {
    ...
    nodes: payload.nodes.map((n) => ({
      ...
      levelReliability: n.level_reliability,
      club: n.club,
      matches: n.matches,
      winRate: n.win_rate,
      since: n.since,
