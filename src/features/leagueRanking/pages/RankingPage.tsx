...
export default function RankingPage() {
  ...
  // A player's id IS the auth uid (players.id == rally_users.user_id), so the session's user
  // id is directly comparable to a row's player_id — the board marks the viewer's own row.
    ...
    <main className="isolate overflow-x-clip pt-32 pb-24">
      ...
      <div className="container relative mx-auto max-w-5xl px-4">
        ...
        <header className="hero-rise hero-rise-1 relative mb-10">
          ...
          {/* Collaboration credit — Tom Bakshi Padel Community collaborates on the
              rating, it does not own it. A muted line, not a masthead: no card, no
              background, no border. The long form lives in the page footer. */}
          <p className="mt-3 flex items-center gap-2 text-sm text-rally-text-2">
            {/* The transparent PNG, trimmed to its content box: the old JPG's mark
                filled 32% of a square canvas, so at h-22 the visible logo was under
                9px. Trimmed and at h-52 it reads at last. The artwork is navy on
                transparent and this page's ground is #18181b, so it is flattened to
                a white silhouette — brand navy on near-black is unreadable, and a
                white plate under it would restore the card this line must not have. */}
            <img
              src="/tom-bakshi.png"
              alt="Tom Bakshi Padel Community"
              className="h-[52px] w-auto [filter:brightness(0)_invert(1)]"
            />
            {t('league.credit.line')}
          </p>
      ...
      {/* The long-form collaboration credit lives here, not beside the hero line:
          the full attribution belongs in the fine print, not above the fold. */}
      <footer className="container relative mx-auto max-w-5xl px-4 pt-10 text-center text-xs text-rally-text-muted">
        {t('league.credit.long')}
      </footer>
...
type StandingsSectionProps = {
  ...
  /** The viewer's own player id, or null — their row's ball link says "find yourself". */
