...
export const StandingsRowSchema = z.object({
  ...
  // Three states, not two (spec §4): true = verified, false = the server
  // checked and said no, null = the server did not say. `.catch(null)` keeps a
  // malformed value in the "did not say" bucket rather than asserting a
  // negative the API never made.
  level_verified: z.boolean().nullable().catch(null),
  level_reliability: z.number().int().nullable().catch(null),
...
/**
 * One frozen tournament award. Deliberately does NOT carry `level_verified` /
 * `level_reliability`: this `band_code` is the band as it stood at award time, not a
 * claim about the player's level today, and the API (`PublicPlayerResult` /
 * `LeagueLedgerEntry`) never sends the pair on this record. Do not add it here.
 */
...
const PlayerContextFields = {
  ...
  // checked and said no, null = the server did not say. `.catch(null)` keeps a
  // malformed value in the "did not say" bucket rather than asserting a
  // negative the API never made. Spread into both PublicPlayerSeasonSchema and
  // MyLeagueCardSchema below, so this one definition covers both — matching
  // rally-api, which carries the pair once per payload, immediately beside
  // `band_code`.
  level_verified: z.boolean().nullable().catch(null),
  level_reliability: z.number().int().nullable().catch(null),
  is_provisional: z.boolean().catch(false),
  level_rank: z.number().nullable().catch(null),
  /** How many players sit on this level's board — the "of N" beside the level rank. */
  level_players: z.number().nullable().catch(null),
  /** Movement on the LEVEL board, which can legitimately disagree with `rank_change`
   *  (the global re-rank). Pairs with `level_rank`, never with `global_rank`. */
  level_rank_change: z.number().nullable().catch(null),
...
export const PublicPlayerSeasonSchema = z.object({
  ...
  rank_change: z.number().nullish(),
  // `level_verified` / `level_reliability` reach this schema through the spread below
  // (`PlayerContextFields`), not as a separate declaration here — the API sends both
  // flat, immediately beside `band_code`, which already lives in that shared block.
  ...PlayerContextFields,
  results: z.array(LeagueResultSchema),
