...
// The modal this file opens renders `PlayerSeasonContent`, which asks `useReadyViewerId()`
// for a viewer — signed out here, like the rest of this file, so no in-network request.
...
describe('TopRanks', () => {
  ...
  it('every card links to the ball pinned on its player, beside (not inside) the card button', () => {
    const onSelect = vi.fn();
    render(
      <MemoryRouter>
        <TopRanks rows={[row(1, 'a', 900), row(2, 'b', 800)]} onSelect={onSelect} viewerId="b" />
      </MemoryRouter>,
    );
    const see = screen.getByRole('link', { name: /see on the ball/i });
    const mine = screen.getByRole('link', { name: /find yourself on the ball/i });
    expect(see).toHaveAttribute('href', '/network?player=a');
    expect(mine).toHaveAttribute('href', '/network?player=b');
    // a link may not nest in a button — and the row's own click must not fire from the link
    expect(see.closest('button')).toBeNull();
    fireEvent.click(see);
    expect(onSelect).not.toHaveBeenCalled();
    // the card's button is still a real button, one per row
