...
describe('v2 fields', () => {
  ...
  it('parses the card context and the quarter blocks, degrading when absent', () => {
    const card = MyLeagueCardSchema.parse({
      ...
      level_rank: 7, level_players: 42, level_rank_change: 2,
      gap_to_above: 25, career_points: 1140,
      quarters: [{
    ...
    expect(card.level_players).toBe(42);
    expect(card.level_rank_change).toBe(2);
    ...
    // Absent, not merely null: the level block degrades to a single reliable
    // `=== null` for every consumer, which is what `.nullable()` (rather than
    // `.nullish()`) buys here — see the note on PlayerContextFields.
    expect(bare.level_players).toBeNull();
    expect(bare.level_rank_change).toBeNull();
  });
