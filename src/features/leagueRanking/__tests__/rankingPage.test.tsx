...
const mockUser = vi.hoisted(
  () => ({ current: null as { id?: string; user_metadata?: Record<string, unknown> } | null }),
);
...
/**
 * This page never draws a seal itself — it hands `myCard.card` and the display-split
 * `featured`/`tableRows` arrays straight to `PersonalCard`/`TopRanks`/`StandingsTable`
 * (see `card={myCard.card}`, `rows={featured}`, `rows={tableRows}` in RankingPage.tsx).
 * `leagueSurfaces.seal.test.ts` pins that those exact expressions survive; THIS is the
 * behavioural proof — it fails the moment someone replaces one of them with a projection
 * that drops `level_verified` (e.g. `rows={featured.map(toDisplayRow)}`), which a source
 * grep alone cannot catch.
 */
describe('RankingPage carries the verification seal through untouched', () => {
  it('a verified row on the featured ladder is sealed', async () => {
    const board = Array.from({ length: 12 }, (_, i) => row(i + 1, `p${i + 1}`, 900 - i));
    board[0] = { ...board[0], level_verified: true };
    vi.mocked(fetchPublicStandings).mockResolvedValue(standings(board));

    renderPage();

    const ladder = await screen.findByTestId('top-ranks');
    expect(within(ladder).getByTestId('verified-seal')).toBeTruthy();
  });

  it('a verified row past the featured cut lands in the table and is still sealed', async () => {
    // FEATURED_COUNT is 10: with 12 rows, rank 12 is display-split into the table, not
    // the ladder — the seal must survive that split too.
    const board = Array.from({ length: 12 }, (_, i) => row(i + 1, `p${i + 1}`, 900 - i));
    board[11] = { ...board[11], level_verified: true };
    vi.mocked(fetchPublicStandings).mockResolvedValue(standings(board));

    renderPage();

    const table = await screen.findByRole('table');
    expect(within(table).getByTestId('verified-seal')).toBeTruthy();
  });

  it('a verified signed-in viewer sees the seal on their own personal card', async () => {
    mockSession.current = { user: { id: 'u1' } };
    mockUser.current = { user_metadata: { full_name: 'Noa Levi' } };
    vi.mocked(fetchPublicStandings).mockResolvedValue(standings([row(1, 'a', 900)]));
    vi.mocked(fetchMyLeagueCard).mockResolvedValue({
    ...
    const card = await screen.findByTestId('league-personal-card');
    expect(within(card).getByTestId('verified-seal')).toBeTruthy();
  });
});

describe('RankingPage — the door to the ball', () => {
  it("marks the signed-in viewer's own row 'find yourself' and every other row 'see on the ball'", async () => {
    mockSession.current = { user: { id: 'me' } };
    mockUser.current = { id: 'me', user_metadata: {} };
    vi.mocked(fetchPublicStandings).mockResolvedValue(
      standings([row(1, 'a', 900), row(2, 'me', 800), row(3, 'c', 700)]),
    );
    vi.mocked(fetchMyLeagueCard).mockResolvedValue({ kind: 'not_found' } as never);

    renderPage();

    await screen.findByTestId('top-ranks');
    const mine = screen.getAllByRole('link', { name: /find yourself on the ball/i });
    expect(mine).toHaveLength(1);
    expect(mine[0]).toHaveAttribute('href', '/network?player=me');
    expect(screen.getAllByRole('link', { name: /see on the ball/i })).toHaveLength(2);
  });

  it('logged out, every row says "see on the ball" — nobody is "yourself"', async () => {
    vi.mocked(fetchPublicStandings).mockResolvedValue(
      standings([row(1, 'a', 900), row(2, 'b', 800), row(3, 'c', 700)]),
    );

    renderPage();

    await screen.findByTestId('top-ranks');
    expect(screen.queryByRole('link', { name: /find yourself on the ball/i })).toBeNull();
    expect(screen.getAllByRole('link', { name: /see on the ball/i })).toHaveLength(3);
  });
});
