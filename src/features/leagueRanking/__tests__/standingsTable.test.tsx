...
// Every row now carries a link to the ball, so the table needs router context.
...
describe('StandingsTable — the door to the ball', () => {
  it('every row links to the ball pinned on that player, without opening the modal', () => {
    const onSelect = vi.fn();
    render(<StandingsTable rows={[row(1, 'a'), row(2, 'b')]} onSelectPlayer={onSelect} />);
    const links = screen.getAllByRole('link', { name: /see on the ball/i });
    expect(links.map(l => l.getAttribute('href'))).toEqual(['/network?player=a', '/network?player=b']);
    fireEvent.click(links[0]);
    expect(onSelect).not.toHaveBeenCalled();
  });

  it("says 'find yourself' on the viewer's own row, and only there", () => {
    render(<StandingsTable rows={[row(1, 'a'), row(2, 'me')]} viewerId="me" />);
    expect(screen.getByRole('link', { name: /find yourself on the ball/i })).toHaveAttribute(
      'href',
      '/network?player=me',
    );
    expect(screen.getAllByRole('link', { name: /see on the ball/i })).toHaveLength(1);
  });

  it('encodes a player id in the link', () => {
    render(<StandingsTable rows={[row(1, 'a b')]} />);
    expect(screen.getByRole('link', { name: /see on the ball/i })).toHaveAttribute('href', '/network?player=a%20b');
  });
});
