...
type StandingsTableProps = {
  ...
  emptyMessage?: string;
  /**
   * The signed-in viewer's auth uid, which IS their player id. Their own row's ball
   * link says "find yourself"; every other row's says "see on the ball".
   */
      ...
      <table className="w-full min-w-[28rem] border-collapse">
        ...
        <thead>
          <tr className="border-b border-rally-border bg-rally-surface-2 text-[11px] font-black uppercase tracking-wider text-rally-text-muted">
            ...
            {/* The ball column has no visible heading — the mark is the heading. */}
            <th scope="col" className="px-2 py-2.5">
              <span className="sr-only">{t('league.ball.see')}</span>
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map(row => (
            ...
            >
              ...
              <td className="px-2 py-2.5 text-end align-middle">
