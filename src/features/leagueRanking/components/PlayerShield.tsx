 ...
 *
 * DELIBERATELY CARRIES NO VERIFICATION SEAL, even though `TopRanks` and
 * `PlayerSeasonContent` both place a shield next to a player who may be
 * `level_verified`. Three reasons, not one:
 *   1. The whole `<svg>` is `aria-hidden` with no text node — a seal marks a
 *      real claim about the player, and putting it inside content hidden from
 *      assistive tech would make that claim unavailable to exactly the users
 *      who most need it stated as text, not just as a pixel.
 *   2. This component's own contract, above, is that it carries no words —
 *      "the row around this component carries the words." Identity (and any
 *      claim about identity) belongs with the name, not the portrait.
 *   3. `TopRanks` already seals the name beside this shield. A second seal on
 *      the shield would double-mark the same player, and a reader seeing two
 *      seals for one row reasonably wonders whether they mean different
 *      things.
 * If a future surface-guard test enumerates every league component that names
 * a player and asserts each one carries (or forwards) the level pair, this
 * component belongs in that guard as an explicit, commented exclusion — not a
 * gap to "fix" by drawing a badge onto the shield.
 *
 * The same three reasons exclude the UNVERIFIED label too: `VerificationMark`'s
 * ghost seal and "Not verified" word are just as much a claim about the player
 * as the seal is, so they belong on the row's own text, not on this hidden,
 * wordless portrait.
 */
