...
export type PlayerIdentitySource = {
  ...
  /**
   * `VerificationMark` owns the true/false/null split: a seal for a confirmed
   * level, a ghost seal for an explicit "not yet", and nothing at all when the
   * server never said (see the component's own doc). `level_reliability` is
   * deliberately not a prop here: nothing in this component shows a
   * reliability affordance.
   */
...
export function PlayerIdentity({ player, className }: PlayerIdentityProps): ReactElement {
  ...
  return (
    <span className={cn('inline-flex min-w-0 items-center gap-2.5', className)}>
      ...
      <VerificationMark verified={player.level_verified} showLabel={false} className="shrink-0" />
    </span>
  );
}
