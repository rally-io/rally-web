...
type TopRanksProps = {
  ...
  /** The signed-in viewer's auth uid (their player id): their own card says "find yourself". */
        ...
        return (
          <li key={row.player_id}>
            <Reveal delay={Math.min(index * 55, 330)}>
              {/* The card is the frame; the button inside it is the row's click and the ball
                  link sits beside the button, never inside it — a link may not nest in a
                  button, and a `div role="button"` would re-own Enter, Space and focus. */}
              <div
                className={cn(
                  'flex items-center gap-1 rounded-xl border bg-rally-surface pe-2 sm:pe-3',
              ...
              <button
                ...
                className={cn(
                  'flex min-w-0 flex-1 items-center gap-3 rounded-xl ps-3 pe-1 text-start sm:gap-4 sm:ps-4',
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  >
                    <span className="min-w-0 truncate">{playerFullName(row) || '—'}</span>
                    <VerificationMark verified={row.level_verified} showLabel={false} className="shrink-0" />
                  </span>
              ...
              <BallLink playerId={row.player_id} isSelf={row.player_id === viewerId} source="ranking_row" />
              </div>
            </Reveal>
          </li>
        );
