...
type QuarterTilesProps = {
  ...
  /**
   * What an unplayed quarter says. Defaults to `league.quarters.empty` — "you did not
   * play" — which is the personal card speaking to its owner. A surface about SOMEBODY
   * ELSE (the public player page) passes its own impersonal wording instead; the same
   * third-person rule that gave the player page `league.player.reasonPlayed`.
   */
  emptyLabel?: string;
  className?: string;
};
 ...
 */
export function QuarterTiles({ quarters, emptyLabel, className }: QuarterTilesProps): ReactElement | null {
          ...
          >
            ...
            <span className="text-[10px] leading-tight text-rally-text-muted">
              {played
                ...
                : (emptyLabel ?? t('league.quarters.empty'))}
            </span>
