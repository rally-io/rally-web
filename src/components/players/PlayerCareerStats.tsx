...
export function PlayerCareerStats({ stats }: { stats: PlayerCareerStatsData }): ReactElement {
  ...
  return (
    <section data-testid="player-season-stats">
      ...
      <p className="mt-1 text-sm text-rally-text-2">{t('league.stats.scope')}</p>
...
function StatTile({
  ...
  return (
    <div className="rounded-xl border border-rally-border bg-rally-surface p-3">
      <dt className="text-xs font-bold uppercase tracking-widest text-rally-text-muted">{label}</dt>
      <dd className="mt-1 flex flex-wrap items-baseline gap-1.5">
        ...
        {sub ? <span className="text-xs text-rally-text-muted">{sub}</span> : null}
      </dd>
    </div>
  )
}
