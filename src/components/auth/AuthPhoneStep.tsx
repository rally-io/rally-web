...
export function AuthPhoneStep({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-4">
    <button type="button" disabled={pending} onClick={onBack} className="text-sm text-rally-text-2">{t('common.back')}</button>
    <p className="text-sm text-rally-text-2">{t('auth.phone.subtitle')}</p>
    ...
    {sent && <>
      <p role="status" className="text-sm text-rally-text-2">{t('auth.phone.sent')}</p>
    ...
    {sent && <button type="button" disabled={pending || cooldown > 0} onClick={send} className="w-full text-sm text-rally-text-2 disabled:text-rally-text-muted">
      {cooldown ? t('auth.verify.resend_cooldown', { seconds: cooldown }) : t('auth.phone.resend')}
    </button>}
    {sent && <button type="button" disabled={pending} onClick={() => { setSent(false); setOtp(''); setError(null) }} className="w-full text-sm text-rally-text-2 underline">{t('auth.phone.change_number')}</button>}
  </form>
}
