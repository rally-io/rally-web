      ...
      {done ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            />
            {validationError && <p role="alert" className="mt-1 text-sm text-rally-error">{validationError}</p>}
          </div>
          <Button type="submit" disabled={pending || !email} className="w-full bg-rally-accent text-rally-accent-text hover:bg-rally-accent-hover">
            {pending ? (t('common.loading') || 'Loading...') : (t('auth.forgot.cta') || 'Send reset link')}
          </Button>
          <button
            type="button"
            onClick={() => navigate(authPath('/login', next, { email }))}
            className="block text-center w-full text-sm text-rally-text-2 hover:text-rally-accent"
          >
            {t('auth.back_to_login') || 'Back to login'}
          </button>
        </form>
