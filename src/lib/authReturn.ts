...
/**
 * `safeReturnTo` for callers that must tell "rejected" apart from "home".
 * The payment pages read `?return_to=` and fall back to their OWN default when
 * it is absent or unsafe — collapsing both onto '/' would silently send a
 * corporate registrant to the site's home page instead of back to /join/<slug>.
 * One validator, two shapes: never re-implement the rules here.
 */
