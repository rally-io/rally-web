...
- **Repo:** `/Users/shahafpariente/Desktop/SideKicks/Rally/rally-web`, branch **`feat/level-verified`, already created off `main` (= `origin/main` @ `46e6806`)** — re-based from `feat/player-globe` on the user's instruction (2026-09-05). `src/features/playerGlobe/` does **not** exist on `main`, so Tasks 8 and 10 and Task 14 Step 1 were **moved out** to `docs/superpowers/plans/2026-09-07-level-verified-globe-followup.md` on 2026-09-07 — they run on the globe branch. This plan has no globe dependency left.
...
| `src/i18n/locales/{en,he}.json` | new `level` namespace; drop four `level_page` table keys (dropping `network.levelChip` moved out with Task 14 Step 1) |
...
| ~~`src/features/playerGlobe/components/PlayerStatsTab.tsx`~~ | **MOVED OUT (Task 8)** → the globe follow-up |
| `src/components/tournaments/ParticipantsSection.tsx` (+ test) | `LevelChip sm` per non-guest player |
| ~~`src/features/playerGlobe/{types.ts, api/network.ts, …}`~~ | **MOVED OUT (Task 10)** → the globe follow-up |
...
- Modify: `src/i18n/locales/en.json` (append namespace after `screenMessages`, the last one on `main` — file currently ends `    }\n  }\n}`)
- Modify: `src/i18n/locales/he.json` (same; on `main` it also ends with a trailing newline)
...
'
```
Expected: `37 same keys` (22 top-level leaves + 15 under `explainer`: `title`, 6×2 block keys, `close`, `readMore`).
...
**Files:**
- Modify: `src/types/api.ts:216-224` (`TournamentParticipantPlayer`), `:322-327` (`PlayerSearchResult`), `:385-394` (`PlayerMe`)
...
`PlayerMe` becomes (on `main` the id key is still declared `id` and `email` is required — keep both exactly as they are; `EditProfilePage.test.tsx:141` builds a `PlayerMe` fixture with `id: 'p1'`, so renaming the key would fail `tsc -b`. The `player_id` / optional-`email` correction lives on `feat/player-globe` and lands with it):
...
**Files:**
- Modify: `src/components/layout/Navbar.tsx` (imports near `:26-29`; the pill at `:245-254`)
...
`skillLevel` (`:128`) stays as is — it is `playerProfile?.skill_level ?? null`.
...
### Task 8: `/network` PlayerStatsTab — self vs others — **MOVED OUT**

> Moved to `docs/superpowers/plans/2026-09-07-level-verified-globe-followup.md` on 2026-09-07. It edits `src/features/playerGlobe/`, which belongs to
> `feat/player-globe`, a different feature branch. Nothing in this plan depends on it.
...
### Task 10: Globe — payload fields, dashed rim, tooltip chip — **MOVED OUT**

> Moved to `docs/superpowers/plans/2026-09-07-level-verified-globe-followup.md` on 2026-09-07, with Task 8.
...
const EXPLAINER_BLOCKS: { key: string; icon: LucideIcon | 'seal' | 'ghost'; values?: Record<string, number> }[] = [
  ...
  { key: 'reliability', icon: Gauge },
  {
    key: 'verified',
    icon: 'seal',
    values: { threshold: ltrIsolate(`${VERIFIED_RELIABILITY_THRESHOLD}%`), matches: TYPICAL_MATCHES_TO_VERIFY, tournaments: TYPICAL_TOURNAMENTS_TO_VERIFY },
  },
  { key: 'counts', icon: ListChecks },
  { key: 'keeping', icon: 'ghost', values: { months: INACTIVITY_GRACE_MONTHS } },
  { key: 'tier', icon: Medal },
...
Expected: PASS; the key-parity check from Task 1 (`node` script) still reports `37 same keys` for `level`, and the two locales' `level_page` blocks have the same keys.
...
### Task 14: Full verification, plan commit

**Files:**
- (`network.levelChip` removal moved out with Tasks 8/10 → `docs/superpowers/plans/2026-09-07-level-verified-globe-followup.md`; the `network` namespace does not exist on `main`.)
...
- [ ] **Step 3: Commit the plan's ticked checkboxes**

The plan is already committed on `feat/level-verified`; this commit only records the
...
- Every place the web printed a level (`Navbar`, participants, partner search, `EditProfilePage`; the `/network` tab + tooltip live in the globe follow-up) now goes through `describeLevel` → `LevelChip` / `ReliabilityRing`; `grep -rn "toFixed(1)" src` returns no level formatting (the remaining hits, if any, are scores or percentages — check each).
- Against a rally-api that serves `level_verified` / `level_reliability` (rally-api plan, Tasks 1–5): the navbar chip shows a seal for a verified account; `EditProfilePage` shows the ring, asks before replacing a verified level, reveals the new one afterward, and its "How is my level calculated?" link opens `LevelExplainerSheet`.
